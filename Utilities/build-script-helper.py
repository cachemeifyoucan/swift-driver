...
def call_output(cmd, cwd=None, stderr=False, verbose=False):
    ...
    try:
        return subprocess.check_output(
            cmd, cwd=cwd, stderr=stderr, universal_newlines=True
        ).strip()
...
def get_dispatch_cmake_arg(args):
    """
    Returns the CMake argument to the Dispatch configuration to use
    for building SwiftPM.
    """
...
def get_foundation_cmake_arg(args):
    """
    Returns the CMake argument to the Foundation configuration to use for
    building SwiftPM.
    """
...
def swiftpm_bin_path(swift_exec, swiftpm_args, env=None):
    swiftpm_args = [
        arg for arg in swiftpm_args if arg != "-v" and arg != "--verbose"
    ]
    ...
    if build_os.startswith("macosx"):
        ...
        if "-android" in args.build_target:
            swiftpm_args += [
                ...
                "-rpath",
                "-Xlinker",
                "$ORIGIN/../lib/swift/android",
            ]
        elif "-freebsd" in args.build_target:
            # Library rpath for swift, dispatch, Foundation, etc.
            # when installing
            swiftpm_args += [
                "-Xlinker",
                "-rpath",
                "-Xlinker",
                "$ORIGIN/../lib/swift/freebsd",
            ]
...
def delete_rpath(rpath, binary, verbose):
    ...
    if verbose:
        print(" ".join(cmd))
    installToolProcess = subprocess.Popen(
        cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
    stdout, stderr = installToolProcess.communicate()
    if installToolProcess.returncode != 0:
        print(
            "install_name_tool -delete_rpath command failed, "
            "assume incremental build and proceed."
        )
    if verbose:
        print(stdout)
...
def add_rpath(rpath, binary, verbose):
    ...
    if installToolProcess.returncode != 0:
        print(
            "install_name_tool -add_rpath command failed, "
            "assume incremental build and proceed."
        )
    if verbose:
        print(stdout)
...
def handle_invocation(args):
    ...
    if args.action == "build":
        has_hosts = args.cross_compile_hosts
        if has_hosts and "-macosx" not in has_hosts[0]:
        ...
        for tool in driver_toolchain_tools:
            ...
            if os.path.exists(tool_path):
                env[
                    "SWIFT_DRIVER_" + tool.upper().replace("-", "_") + "_EXEC"
                ] = "%s" % (tool_path)
...
# Installation flow for non-darwin platforms, only copies over swift-driver
# and swift-help.
...
def install(args, build_dir, targets):
    ...
    # and libSwiftDriver, libSwiftOptions libraries, along with their
    # dependencies.
    for prefix in args.install_prefixes:
...
def install_swiftdriver(args, build_dir, prefix, targets):
    ...
    universal_dir = os.path.join(
        build_dir, "universal-apple-macos%s" % macos_deployment_target
    )
...
# Install universal binaries for swift-driver and swift-help into the
# toolchain bin directory
def install_executables(
    args, build_dir, universal_bin_dir, toolchain_bin_dir, targets
):
    for exe in executables_to_install:
        # Fixup rpaths
        for target in targets:
            exe_bin_path = os.path.join(
                build_dir, target, args.configuration, "bin", exe
            )
            driver_lib_dir_path = os.path.join(
                build_dir, target, args.configuration, "lib"
            )
        ...
        # Merge the multiple architecture binaries into a universal binary
        # and install
...
# Install shared libraries for the driver and its dependencies into
# the toolchain
def install_libraries(
    args, build_dir, universal_lib_dir, toolchain_lib_dir, targets
):
    # Fixup the SwiftDriver rpath for libSwiftDriver and
    # libSwiftDriverExecution
    ...
    tsc_libs = [
        os.path.join("dependencies", "swift-tools-support-core", "lib", d)
        for d in ["libTSCBasic", "libTSCUtility"]
    ]
    for lib in driver_libs + tsc_libs:
        for target in targets:
            lib_path = os.path.join(
                build_dir, target, args.configuration, lib + shared_lib_ext
            )
    ...
    # Install the libSwiftDriver and libSwiftOptions and
    # libSwiftDriverExecution shared libraries into the toolchain lib
    ...
    # Install the swift-tools-support core shared libraries into the
    # toolchain lib
    package_subpath = os.path.join(
        args.configuration, "dependencies", "swift-tools-support-core"
    )
    ...
    package_subpath = os.path.join(
        args.configuration, "dependencies", "swift-argument-parser"
    )
    for lib, ext in [
        ("libArgumentParser", shared_lib_ext),
        ("libArgumentParserToolInfo", static_lib_ext),
    ]:
        install_library(
            args,
            build_dir,
    ...
    package_subpath = os.path.join(
        args.configuration, "dependencies", "llbuild"
    )
...
def install_library(
    ...
    for target in targets:
        input_lib_path = os.path.join(
            build_dir, target, package_subpath, "lib", lib_file
        )
        lipo_cmd.append(input_lib_path)
    lipo_cmd.extend(["-create", "-output", output_dylib_path])
    subprocess.check_call(lipo_cmd)
    install_binary(
        lib_file, universal_lib_dir, toolchain_lib_dir, args.verbose
    )


# Install binary .swiftmodule files for the driver and its dependencies into
# the toolchain lib
def install_binary_swift_modules(args, build_dir, toolchain_lib_dir, targets):
    # The common subpath from a project's build directory to where its build
    # products are found
...
# Install the modulemaps and headers of the driver's C module dependencies
# into the toolchain include directory
def install_c_module_includes(args, build_dir, toolchain_include_dir):
    # TSCclibc C module's modulemap and header files
    tscc_include_dir = os.path.join(
        os.path.dirname(args.package_path),
        "swift-tools-support-core",
        "Sources",
        "TSCclibc",
        "include",
    )
    install_include_artifacts(
        args, toolchain_include_dir, tscc_include_dir, "TSCclibc"
    )


def install_module(
    args, build_dir, package_subpath, toolchain_lib, module_name, targets
):
    toolchain_module_dir = os.path.join(
        toolchain_lib, module_name + ".swiftmodule"
    )
    mkdir_p(toolchain_module_dir)
    for target in targets:
        swift_dir = os.path.join(build_dir, target, package_subpath)
        for fileext in [".swiftmodule", ".swiftdoc"]:
            ...
            os.rename(
                os.path.join(toolchain_module_dir, module_name + fileext),
                os.path.join(toolchain_module_dir, target + fileext),
            )


# Copy over the contents of a module's include directory contents
# (modulemap, headers, etc.)
def install_include_artifacts(
    args, toolchain_include_dir, src_include_dir, dst_module_name
):
    toolchain_module_include_dir = os.path.join(
        toolchain_include_dir, dst_module_name
    )
...
def build_using_cmake(args, toolchain_bin, build_dir, targets):
    ...
    base_swift_flags.append(
        '-module-cache-path "{}"'.format(
            os.path.join(build_dir, "module-cache")
        )
    )
    ...
    for target in targets:
        ...
        if "-apple-macosx" in args.build_target:
            base_cmake_flags.append(
                "-DCMAKE_OSX_DEPLOYMENT_TARGET=%s" % macos_deployment_target
            )
            base_cmake_flags.append(
                "-DCMAKE_OSX_ARCHITECTURES=%s" % target.split("-")[0]
            )

        # Target directory for build artifacts
        # If building for a local compiler build, use the build directory
        # directly
        ...
        # Argument Parser
        build_argument_parser_using_cmake(
            args,
            target,
            swiftc_exec,
            dependencies_dir,
        ...
        # SwiftDriver
        build_swift_driver_using_cmake(
            args,
            target,
            swiftc_exec,
            driver_dir,
...
def build_llbuild_using_cmake(
    args, target, swiftc_exec, build_dir, base_cmake_flags, swift_flags
):
    print("Building Swift Driver dependency: llbuild")
    llbuild_source_dir = os.path.join(
        os.path.dirname(args.package_path), "llbuild"
    )
    ...
    subprocess.check_call(
        ["touch", os.path.join(llbuild_api_dir, "codemodel-v2")]
    )
    flags = [
    ...
    if args.sysroot:
        llbuild_cmake_flags.append(
            "-DSQLite3_INCLUDE_DIR=%s/usr/include" % args.sysroot
        )
        # FIXME: This may be particularly hacky but CMake finds a different
        # version of libsqlite3 on some machines.
        # This is also Darwin-specific...
        if "-apple-macosx" in args.build_target:
            llbuild_cmake_flags.append(
                "-DSQLite3_LIBRARY=%s/usr/lib/libsqlite3.tbd" % args.sysroot
            )
...
def build_tsc_using_cmake(
    args, target, swiftc_exec, build_dir, base_cmake_flags, swift_flags
):
    print("Building Swift Driver dependency: TSC")
    tsc_source_dir = os.path.join(
        os.path.dirname(args.package_path), "swift-tools-support-core"
    )
...
def build_argument_parser_using_cmake(
    args, target, swiftc_exec, build_dir, base_cmake_flags, swift_flags
):
    print("Building Swift Driver dependency: Argument Parser")
    parser_source_dir = os.path.join(
        os.path.dirname(args.package_path), "swift-argument-parser"
    )
...
def build_swift_driver_using_cmake(
    args, target, swiftc_exec, build_dir, base_cmake_flags, swift_flags
):
    ...
    llbuild_dir = os.path.join(dependencies_dir, "llbuild")
    tsc_dir = os.path.join(dependencies_dir, "swift-tools-support-core")
    argparser_dir = os.path.join(dependencies_dir, "swift-argument-parser")
    flags = [
        "-DLLBuild_DIR=%s" % os.path.join(llbuild_dir, "cmake/modules"),
        "-DTSC_DIR=%s" % os.path.join(tsc_dir, "cmake/modules"),
        "-DArgumentParser_DIR=%s" % os.path.join(
            argparser_dir, "cmake/modules"
        ),
    ]
...
def cmake_build(
    ...
    cmd = cmd + cmake_args + [source_path]
    if args.verbose:
        print(" ".join(cmd))
    mkdir_p(build_dir)
    ...
    # default encoding. Some systems have a default encoding of 'ascii', but
    # that conflicts with this output, which can contain UTF encoded
    # characters. The bytes are then written, instead of printed, to bypass
    # issues with encoding.
    ninjaProcess = subprocess.Popen(
        ninja_cmd,
        cwd=build_dir,
...
def get_build_target(swiftc_path, args, cross_compile=False):
    ...
    try:
        ...
        error(
            f"Command {e.cmd} terminated with "
            f"non-zero exit status {e.returncode}"
            f"\n  Output: {e.output}"
        )
    except Exception as e:
        error(str(e))
...
def main():
    parser = argparse.ArgumentParser(
        description="Build along with the Swift build-script."
    )
    ...
    def add_common_args(parser):
        parser.add_argument(
            "--package-path",
            metavar="PATH",
            help="directory of the package to build",
            default=".",
        )
        parser.add_argument(
            "--toolchain",
            required=True,
            metavar="PATH",
            help="build using the toolchain at PATH",
        )
        parser.add_argument(
            "--prefix",
            dest="install_prefixes",
            nargs="*",
            help="paths (relative to the project root) where to install "
            "build products [%(default)s]",
            metavar="PATHS",
        )
        parser.add_argument(
            "--cross-compile-hosts",
        ...
        parser.add_argument(
            ...
            metavar="PATH",
            help="A JSON SPM config file with Swift flags for "
            "cross-compilation",
        )
        parser.add_argument(
            "--ninja-bin",
            metavar="PATH",
            help="ninja binary to use for testing",
        )
        parser.add_argument(
            "--cmake-bin",
            metavar="PATH",
            help="cmake binary to use for building",
        )
        parser.add_argument(
            "--build-path",
            metavar="PATH",
            default=".build",
            help="build in the given path",
        )
        parser.add_argument(
            "--foundation-build-dir",
            metavar="PATH",
            help="Path to the Foundation build directory",
        )
        parser.add_argument(
            "--dispatch-build-dir",
            metavar="PATH",
            help="Path to the Dispatch build directory",
        )
        parser.add_argument(
            "--lit-test-dir",
            metavar="PATH",
            help="the test dir in the Swift build directory",
        )
        parser.add_argument(
            "--configuration",
            "-c",
            default="debug",
            help="build using configuration (release|debug)",
        )
        parser.add_argument(
            "--no-local-deps",
            action="store_true",
        ...
        parser.add_argument(
            ...
            action="store_true",
            help="driver is being built for use with a local compiler build",
        )
        parser.add_argument(
            "--enable-asan",
            action="store_true",
    ...
    args.build_target = get_build_target(
        swift_exec,
        args,
        cross_compile=(True if args.cross_compile_config else False),
    )
    if "-apple-macosx" in args.build_target:
        args.sysroot = call_output(
    ...
    is_x86 = args.build_target == "x86_64-apple-macosx"
    is_arm = args.build_target == "arm64-apple-macosx"
    cross = args.cross_compile_hosts
    if is_x86 and "macosx-arm64" in cross:
        args.cross_compile_hosts = [
            args.build_target + macos_deployment_target,
            "arm64-apple-macosx%s" % macos_deployment_target,
        ]
    elif is_arm and "macosx-x86_64" in cross:
        args.cross_compile_hosts = [
            args.build_target + macos_deployment_target,
            "x86_64-apple-macosx%s" % macos_deployment_target,
        ]
    elif (
        args.cross_compile_hosts and "android-" in args.cross_compile_hosts[0]
    ):
        print("Cross-compiling for %s" % args.cross_compile_hosts[0])
    elif args.cross_compile_hosts:
        error("cannot cross-compile for %s" % args.cross_compile_hosts)

    if args.cross_compile_hosts and args.local_compiler_build:
        error(
            "Cross-compilation is currently not supported for "
            "the local compiler installation"
        )
    ...
    # If a separate prefix has not been specified, installed into the
    # specified toolchain
