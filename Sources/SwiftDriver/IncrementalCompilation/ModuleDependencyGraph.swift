...
/*@_spi(Testing)*/ public final class ModuleDependencyGraph: InternedStringTableHolder {
  ...
  func setExternalModulePathMap(_ map: [String: String]) {
    mutationSafetyPrecondition()
...
extension ModuleDependencyGraph {
  ...
  fileprivate struct ExternalDependencyCurrencyCache {
    ...
    /// Maps relative module paths to absolute paths for filesystem checks.
    var externalModulePathMap: [String: String] = [:]
    ...
    /// path map for relative/abstract paths.
    private func resolvedPath(for externalDependency: ExternalDependency) -> VirtualPath? {
      if let resolvedPathString = externalModulePathMap[externalDependency.fileNameString],
         let resolvedPath = try? VirtualPath(path: resolvedPathString) {
        return resolvedPath
      }
...
extension ModuleDependencyGraph {
  ...
  /// - Minor number 6: externalModulePathNode for resolving relative module paths.
  ...
  @_spi(Testing) public static func deserialize(
    ...
    struct Visitor: BitstreamVisitor, IncrementalCompilationSynchronizer {
      ...
      private var externalModulePathMap = Dictionary<String, String>()
      ...
      mutating func visit(record: BitcodeElement.Record) throws {
        ...
        case .externalModulePathNode:
          ...
          externalModulePathMap[abstractPath.lookup(in: internedStringTable)] =
            resolvedPath.lookup(in: internedStringTable)
        }
      }
    }
...
extension ModuleDependencyGraph {
  ...
  @_spi(Testing) public final class Serializer: InternedStringTableHolder {
    ...
    private func populateCaches(from graph: ModuleDependencyGraph) {
      ...
      for (abstractPath, resolvedPath) in graph.currencyCache.externalModulePathMap {
        _ = abstractPath.intern(in: self.internedStringTable)
        _ = resolvedPath.intern(in: self.internedStringTable)
      }
    ...
    public static func serialize(
      ...
      serializer.stream.withSubBlock(.firstApplicationID, abbreviationBitWidth: 8) {
        ...
        for (abstractPath, resolvedPath) in graph.currencyCache.externalModulePathMap {
          ...
          let resolvedInternedString = resolvedPath.intern(in: graph.internedStringTable)
